...
This script:
  ...
  4) Selects a per-outcome probability threshold maximizing F1 score on a
     held-out validation set (separate from the test set)
...
# ============================================================
# 6. Uncertainty helper
#    Linear transformation: 0 = confident (p near 0 or 1),
#                           1 = maximally uncertain (p = 0.5)
# ============================================================
def uncertainty_from_proba(p: float) -> float:
    """Scaled uncertainty proxy in [0, 1]: u = 1 - 2*|p - 0.5|."""
    return float(1.0 - abs(float(p) - 0.5) * 2.0)


# ============================================================
# 7. Train one model per disease
#    Threshold selection uses a held-out validation set carved
#    from the training data, keeping the test set uncontaminated.
# ============================================================
disease_models = {}
...
for col in target_cols_binary:
    ...
    # Primary train / test split (test set is never used for threshold selection)
    ...
    # Secondary split: carve a validation set from training data for threshold tuning
    ...
    print(f"  Best threshold (validation set) for {col}: {best_thresh:.3f}  "
          f"(validation F1 = {best_f1_val:.3f})")
    ...
    print(f"\nClassification report (optimal threshold = {best_thresh:.3f}):")
...
# ============================================================
# 9. Compare fixed vs optimised thresholds
# ============================================================
...
#     Uses quantile-based (equal-frequency) bins throughout,
#     consistent with make_manuscript_outputs.py and manuscript text.
# ============================================================
def plot_calibration_for_disease(disease, disease_models=disease_models, n_bins=10):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / f"calibration_{disease}.pdf", dpi=300)
    plt.close()
...
# ============================================================
optimal_thresholds = {
    disease: info["best_threshold"] for disease, info in disease_models.items()
}
